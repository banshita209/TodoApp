        ...
        if (todoList.length > 0) {
            return <div className="flex justify-around items-center">
                <div className="text-xl">
                    You are making a great progress!!</div>

                <div className="rounded-full border-4 p-8">
                    <span className="text-white text-2xl">{todoList.filter((todoItem) => todoItem.status === Status.Completed).length}/{todoList.length}</span>
                </div>
            </div>
        } else {
            return <h1 className="italic font-bold text-white">Looks like u dont have task </h1>
        }
    }
    return (
